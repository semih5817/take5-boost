...
const PackCard = ({
  ...
  const displayPrice =
    billing === "free" ? pack.monthly : billing === "monthly" ? pack.monthly : selectedOption;

  return (
    <div
      className={`relative bg-slate-800/50 backdrop-blur border rounded-2xl p-6 transition-all hover:scale-105 ${
        recommended
          ? "border-purple-500 border-2 shadow-2xl shadow-purple-500/50"
          : "border-slate-700"
      }`}
    >
      {/* Badge recommandé */}
      ...
      <ul className="space-y-3 mb-8">
        {pack.features.map((feature, idx) => (
          <li key={idx} className="flex items-start gap-3">
            ...
            (feature.included === "annual" && billing === "annual") ? (
              <Check className="w-5 h-5 text-green-400 mt-0.5 flex-shrink-0" />
            ) : feature.included === false ? (
              <X className="w-5 h-5 text-slate-600 mt-0.5 flex-shrink-0" />
            ) : (
              <Minus className="w-5 h-5 text-slate-600 mt-0.5 flex-shrink-0" />
            )}
      ...
      <Button
        ...
        className={`w-full py-4 rounded-lg font-bold text-white transition-all transform hover:scale-105 ${pack.ctaStyle}`}
      >
        {pack.cta}
      </Button>
...
export const PricingSection = () => {
  ...
  const packs: Record<string, Pack> = {
    free: {
      ...
      monthly: { price: 0, duration: "1 mois", nfcPlates: 0 },
      features: [
    ...
    starter: {
      ...
      features: [
        { text: "Tout du pack FREE, en illimité", included: true },
        { text: "Support prioritaire", included: true },
        { text: "Plaque NFC offerte", included: "annual" },
      ],
      cta: "Démarrer",
    ...
    pro: {
      ...
      features: [
        { text: "Tout du Starter +", included: true },
        { text: "Réponses IA illimitées", included: true },
        { text: "🎨 Dashboard Analytics (démo)", included: true, badge: "🎨 Aperçu" },
        { text: "Alertes concurrents quotidiennes", included: true },
        { text: "Opportunités IA quotidiennes", included: true },
        { text: "Support prioritaire 24/7", included: true },
        { text: "🎮 Jeux concours", included: false, badge: "🔜 T1 2026" },
        { text: "📧 Campagnes SMS/Email", included: false, badge: "🔜 T2 2026" },
      ],
      cta: "Passer au Pro",
      ctaStyle:
    ...
    <section className="py-20 px-4 bg-gradient-to-b from-[#1A1F35] to-[#0A0E1A]">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center mb-12">
          ...
          <div className="inline-flex items-center gap-3 bg-slate-800 rounded-full p-1 border border-slate-700">
            <button
            ...
            >
              Annuel
              <Badge className="bg-green-500 text-white text-xs px-2 py-0.5">-25%</Badge>
            </button>
          </div>
        </div>
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          <PackCard pack={packs.free} billing="free" />
          <PackCard
            pack={packs.starter}
            billing={billingCycle}
            options={billingCycle === "annual" ? packs.starter.annual : null}
          />
          <PackCard
            pack={packs.pro}
            billing={billingCycle}
            options={billingCycle === "annual" ? packs.pro.annual : null}
            recommended={true}
          />
        </div>
