...
export const MultiPublicationAnimation = () => {
  ...
  const posts = [
    ...
    },
    {
      ...
      icon: '🎵',
      color: '#000000',
      caption: 'Tasty Crousty à 9,90€ ce samedi 🔥 #foodtiktok #nancy #tastycrousty'
    }
  ];
  ...
  useEffect(() => {
    const timers = [
      ...
      setTimeout(() => setStep(3), 4500),  // Publications (swipe commence)
      setTimeout(() => setStep(0), 10000), // Reset
    ];
    return () => timers.forEach(clearTimeout);
  }, [step === 0]);

  // Swipe entre les 4 posts
  useEffect(() => {
    if (step === 3) {
      const interval = setInterval(() => {
        setSwipeIndex(prev => (prev + 1) % 4);
      }, 1500);
              ...
              <div className="px-5 animate-in fade-in duration-500">
                <div className="bg-white rounded-2xl p-3 shadow-sm inline-block">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm text-gray-600">TakeFive génère vos publications...</span>
                  </div>
                </div>
              </div>
            )}
